...
import { generateInfo4TaskXML } from '../../utils/xmlGenerator';
...
export default function Layout({
    ...
    const [manualLoadAttempted, setManualLoadAttempted] = useState(false);
    ...
    const [mapShouldClear, setMapShouldClear] = useState(false);
    ...
    useEffect(() => {
        if (route4VehicleUrl) {
            console.log("Saving route4VehicleUrl to localStorage:", route4VehicleUrl);
    ...
    const getAlgorithmEndpoint = (algorithm) => {
        switch (algorithm) {
            case 'ALNS':
                return 'http://157.230.17.89:8005/start_alns';
            case 'SA':
                return 'http://157.230.17.89:8005start_sa'; //ip değişimi algoritma kullanımına göre değişebilir
            case 'TS':
                return 'http://157.230.17.89:8005/start_ts'; //ip değişimi algoritma kullanımına göre değişebilir
            case 'OR':
            default:
                return 'http://157.230.17.89:8005/start_ortools'; //ip değişimi algoritma kullanımına göre değişebilir
        } 
    }; 
        ...
        try {
            ...
            if (response.headers['content-type'] === 'application/zip') {
                ...
                if (saveResult) {
                    ...
                    console.log("Ayrıştırılan dosyalar:", extractedFiles);
                    console.log("Route4Vehicle.json dosyası:", route4VehiclePath);
                    console.log("Route4Plan.xml dosyası:", route4PlanPath);
    ...
    // BURAYI YÜ YAZDI, NİHAİ HALİNDE BURASI ÇALIŞMALI
    const processScenarioWithSelectedOrder = async (scenario) => {
        try {
            setProcessingScenario(scenario);
            ...
            const filesToFetch = [
                { 
                    path: `/xml_dataset/Info4Task/Task.xml`, 
                    name: `Task.xml` 
                },
                { 
                    path: '/xml_dataset/Info4Environment/Info4Environment.xml', 
                    name: 'Info4Environment.xml' 
                },
                { 
                    path: '/xml_dataset/Info4ChargingStation/Info4ChargingStation.xml', 
                    name: 'Info4ChargingStation.xml' 
                },
                { 
                    path: '/xml_dataset/Info4Vehicle/Info4Vehicle.xml', 
                    name: 'Info4Vehicle.xml' 
                },
                { 
                    path: '/xml_dataset/Map4Environment/Map4Environment.xml', 
                    name: 'Map4Environment.xml' 
                },
                { 
                    path: '/xml_dataset/Input4Algorithm/Input.xml', 
                    name: 'Input.xml' 
                }
            ];
    ...
    const handleDeleteFiles = async () => {
        ...
        try {
            ...
            // Trigger map clear
            setMapShouldClear(true);
            
            // Reset map clear trigger after a short delay
            setTimeout(() => {
                setMapShouldClear(false);
            }, 100);
    ...
    const handleProcessSelectedCustomers = async () => {
        ...
        try {
            ...
            // First, fetch matrix positions
            const matrixResponse = await axios.get('http://localhost:3001/api/matrix-positions');
            const matrixPositions = matrixResponse.data;
            
            console.log('Fetched matrix positions:', matrixPositions);

            // Clear local output files
            await axios.post('http://localhost:3001/clear-output');
            await axios.post('http://localhost:3001/clear-task');
            console.log('Output directory cleared successfully');

            // Generate XML with matrix positions
            const xmlContent = generateInfo4TaskXML(selectedCustomers, matrixPositions);
            console.log('Generated XML content:', xmlContent);

            // Save XML file
            const saveXmlResponse = await axios.post('http://localhost:3001/save-xml-info4task', 
                { xmlContent },
                { headers: { 'Content-Type': 'application/json' }}
            );

            if (!saveXmlResponse.data.success) {
                throw new Error('Failed to save XML file');
            }

            console.log('XML saved successfully:', saveXmlResponse.data);

            // Process scenarios with the saved XML
            const totalScenarios = availableScenarios.length;
            setScenarioProgress({ current: 0, total: totalScenarios });
            ...
            for (let i = 0; i < totalScenarios; i++) {
                ...
                const success = await processScenarioWithSelectedOrder(scenario);
                if (success) {
                    hasSuccessfulScenario = true;
                }
            ...
            if (hasSuccessfulScenario) {
                ...
                throw new Error('No scenarios were successfully processed');
            }

        } catch (error) {
            console.error('Error processing customers:', error);
            setOptimizationError(error.message || 'Failed to process customers');
        } finally {
    ...
    // Yeni fonksiyon: Route4Vehicle.json dosyasını manuel olarak yükleme
    const handleManualLoadRoute = async () => {
        setManualLoadAttempted(true);
        try {
            // Önce output dizinindeki dosyaları listele
            const response = await axios.get('http://localhost:3001/list-extracted-files');
            const files = response.data.files;
            
            // Route4Vehicle.json dosyasını bul
            const route4VehiclePath = files.find(path => path.includes('Route4Vehicle.json'));
            ...
            if (route4VehiclePath) {
                ...
                try {
                    ...
                    setRoute4VehicleUrl(url);
                    localStorage.setItem('route4VehicleUrl', url);
                    console.log('Route file manually loaded:', url);
                    return true;
                } catch (error) {
    ...
    // View mode toggle fonksiyonunu güncelle
    const handleViewModeToggle = async () => {
        if (viewMode === "normal") {
            if (!route4VehicleUrl) {
                // URL yoksa manuel yüklemeyi dene
                const success = await handleManualLoadRoute();
                if (!success) {
                    alert('Could not load route file. Please try again or check if optimization was successful.');
                    return;
                }
            }
            setViewMode("route4vehicle");
        } else {
            setViewMode("normal");
        }
    };
        ...
        <div style={{ 
            ...
            <div style={{ 
                ...
                <div style={{ 
                    ...
                    <CustomerPool customers={customers} onCustomerSelect={handleCustomerSelect} />
                </div>
                ...
                <Card style={{ 
                    ...
                    <div style={{ display: "flex", gap: "20px", justifyContent: "center" }}>
                        ...
                        {/* Rota görüntüleme butonu - Güncellendi */}
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginLeft: "15px" }}>
                            <IconButton 
                                color="info" 
                            ...
                            <Typography variant="caption" style={{ fontSize: "14px", color: "#555", marginTop: "2px" }}>
                                {viewMode === "normal" ? 
                                    (route4VehicleUrl ? "Show Route Map" : "Load & Show Route") : 
                                    "Show Normal Map"}
                            </Typography>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginLeft: "15px" }}>
                        ...
                        {/* New Process Selected Customers button */}
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                            <IconButton
                                type="button"
                                color="secondary"
                                onClick={handleProcessSelectedCustomers}
